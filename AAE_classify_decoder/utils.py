...
def load_checkpoint_main(checkpoint, model_gen, model_disc, model_decoder):
    print("----> Loading checkpoint")
    model_gen.load_state_dict(checkpoint['gen_state_dict'])
    model_disc.load_state_dict(checkpoint['disc_state_dict'])
    model_decoder.load_state_dict(checkpoint['decoder_state_dict'])
...
def get_digit_distribution(digits, num_elements):
    ...
    shift = np.multiply(shifts, [4 * max(sx, sy), 4 * max(sx, sy)])
...
def save_latent_distribution(loader, encoder, epoch_num, mode):
    ...
    with torch.no_grad():
        for image, label in loader:
            ...
            latent_vector = latent_vector.squeeze(3).squeeze(2).cpu().numpy()
...
def plot_gmm(num_classes=config.NUM_CLASSES):
    ...
    for digit in range(num_classes):
        ...
        shift = np.multiply(shifts[digit], [4*max(sx, sy), 4*max(sx, sy)])
...
def check_accuracy(loader, encoder, decoder):
    true_predictions = 0
    for batch_idx, (image, label) in enumerate(loader):
        ...
        latent_vector = encoder(image)
        _, labels_softVector = decoder(latent_vector)
        predicted_label = torch.argmax(labels_softVector, dim=1)
        true_predictions += torch.sum(predicted_label == label)

    acc = true_predictions/len(loader)
    print(f"Accuracy of test-dataset is: {acc:.2f}%")
